...
half4 fragForwardBaseInternal (VertexOutputForwardBase i)
{
	...
	if (_SrcBlend == 1)
		return VolumetricTransparencyBase(OutputForward(c, s.alpha), i.projPos);
	else
		//return OutputForward(c, s.alpha);
		return VolumetricTransparency(OutputForward(c, s.alpha), i.projPos);
...
#ifdef VTRANSPARENCY_ON
	return VolumetricTransparencyAdd(OutputForward(c, s.alpha), i.projPos);
#else
	return OutputForward(c, s.alpha);
#endif
}
