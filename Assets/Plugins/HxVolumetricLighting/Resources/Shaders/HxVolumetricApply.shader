...
#include "UnityCG.cginc"
	...
	float4 fragFull(v2ff input) : SV_Target
	{
		...
		float random = nrand(input.uv) * 0.00390625f * 0.30;

		fogSample += float4(random.xxxx);

		return (float4(fogSample.r, fogSample.g, fogSample.b, 0)) + (tex2D(_MainTex, input.uv.xz) * (1 - fogSample.a));
	}
		...
		float4 fragFullGamma(v2ff input) : SV_Target
	{
		...
		float random = nrand(input.uv) * 0.00390625f;

		half4 ScreenColor = tex2D(_MainTex, input.uv.xz);

		return float4(pow(random.xxx + fogSample.rgb + pow(ScreenColor.rgb,2.2) * (1 - fogSample.a),0.4545f), ScreenColor.a);
