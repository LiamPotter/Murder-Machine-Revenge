...
#include "UnityCG.cginc"
		...
		float4 AddResultFull(inout float totalWeight, float centralDepth, float2 uv, float weight, bool isSky)
		{
			...
			return tex2Dlod(_FullVolumetric, float4(uv, 0, 0)) * w;
		}
		...
		{		
			...
			float centralDepthFull = (tex2Dlod(VolumetricDepth, float4(input.uv, 0, 0)).r);
			...
			result += AddResult(totalWeight, centralDepthFull, input.uv + float2(-_MainTex_TexelSize.x * 2,-_MainTex_TexelSize.y * 2), 0.005865, isSky);
			result += AddResult(totalWeight, centralDepthFull, input.uv + float2(-_MainTex_TexelSize.x,-_MainTex_TexelSize.y * 2), 0.018686, isSky);
			result += AddResult(totalWeight, centralDepthFull, input.uv + float2(0,-_MainTex_TexelSize.y * 2), 0.027481, isSky);
			result += AddResult(totalWeight, centralDepthFull, input.uv + float2(_MainTex_TexelSize.x,-_MainTex_TexelSize.y * 2), 0.018686, isSky);
			result += AddResult(totalWeight, centralDepthFull, input.uv + float2(_MainTex_TexelSize.x * 2,-_MainTex_TexelSize.y * 2), 0.005865, isSky);
					  
			result += AddResult(totalWeight, centralDepthFull, input.uv + float2(-_MainTex_TexelSize.x * 2,-_MainTex_TexelSize.y), 0.018686, isSky);
			result += AddResult(totalWeight, centralDepthFull, input.uv + float2(-_MainTex_TexelSize.x,-_MainTex_TexelSize.y), 0.059536, isSky);
			result += AddResult(totalWeight, centralDepthFull, input.uv + float2(0,-_MainTex_TexelSize.y), 0.087555, isSky);
			result += AddResult(totalWeight, centralDepthFull, input.uv + float2(_MainTex_TexelSize.x,-_MainTex_TexelSize.y), 0.059536, isSky);
			result += AddResult(totalWeight, centralDepthFull, input.uv + float2(_MainTex_TexelSize.x * 2,-_MainTex_TexelSize.y), 0.018686, isSky);
					 
			result += AddResult(totalWeight, centralDepthFull, input.uv + float2(-_MainTex_TexelSize.x * 2,0), 0.027481, isSky);
			result += AddResult(totalWeight, centralDepthFull, input.uv + float2(-_MainTex_TexelSize.x,0), 0.087555, isSky);

			result += (tex2Dlod(_MainTex, float4((input.uv).xy, 0, 0))) * 0.12876;
		...
		float4 fragFullGaussian (v3f input) : SV_Target
		{
			...
			result += (tex2Dlod(_FullVolumetric, float4(uv, 0, 0))) * 0.12876;
		...
		{		
			...
			result += AddResult(totalWeight, centralDepthFull, input.uv + float2(-_MainTex_TexelSize.x * 2,-_MainTex_TexelSize.y * 2), 1, isSky);
			result += AddResult(totalWeight, centralDepthFull, input.uv + float2(-_MainTex_TexelSize.x,-_MainTex_TexelSize.y * 2), 1, isSky);
			result += AddResult(totalWeight, centralDepthFull, input.uv + float2(0,-_MainTex_TexelSize.y * 2), 1, isSky);
			result += AddResult(totalWeight, centralDepthFull, input.uv + float2(_MainTex_TexelSize.x,-_MainTex_TexelSize.y * 2), 1, isSky);
			result += AddResult(totalWeight, centralDepthFull, input.uv + float2(_MainTex_TexelSize.x * 2,-_MainTex_TexelSize.y * 2), 1, isSky);

			result += AddResult(totalWeight, centralDepthFull, input.uv + float2(-_MainTex_TexelSize.x * 2,-_MainTex_TexelSize.y), 1, isSky);
			result += AddResult(totalWeight, centralDepthFull, input.uv + float2(-_MainTex_TexelSize.x,-_MainTex_TexelSize.y), 1, isSky);
			result += AddResult(totalWeight, centralDepthFull, input.uv + float2(0,-_MainTex_TexelSize.y), 1, isSky);
			result += AddResult(totalWeight, centralDepthFull, input.uv + float2(_MainTex_TexelSize.x,-_MainTex_TexelSize.y), 1, isSky);
			result += AddResult(totalWeight, centralDepthFull, input.uv + float2(_MainTex_TexelSize.x * 2,-_MainTex_TexelSize.y), 1, isSky);

			result += AddResult(totalWeight, centralDepthFull, input.uv + float2(-_MainTex_TexelSize.x * 2,0), 1, isSky);
			result += AddResult(totalWeight, centralDepthFull, input.uv + float2(-_MainTex_TexelSize.x,0), 1, isSky);

			result += (tex2Dlod(_MainTex, float4((input.uv).xy, 0, 0)));
		...
		float4 fragFullAverage(v3f input) : SV_Target
		{
			float2 uv = input.uv.xy / input.uv.w;
			...
			result += (tex2Dlod(_FullVolumetric, float4(uv, 0, 0)));
