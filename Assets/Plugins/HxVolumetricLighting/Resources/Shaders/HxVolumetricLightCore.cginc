...
#define TS_12 //TS_8, TS_12 , TS_16 
#define DS_16//DS_8, DS_12 , DS_16
...
#ifdef NOISE_ON
	x += NoiseOffset;
	return saturate(tex3Dlod(NoiseTexture3D, float4(x.x * NoiseScale.x, x.y * NoiseScale.y, x.z * NoiseScale.z, 1)).a) * 2;
#else
	return 1;
#endif
}
...
float DensityToPoint(DensityMap dm, float worldDistance)
{
	...
	float density = AddDensity(dm.SliceData[0], dm.SliceData[1], DensitySliceDistance0.x, 0, slice) +
		...
		AddDensity(dm.SliceData[3], dm.SliceData[4], DensitySliceDistance1.z, 3, slice) +
