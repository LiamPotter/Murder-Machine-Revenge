...
float SampleShadows(float2 suv,float zdis)
{
			...
			float z_buffer_value = (ShadowBias.x) + ShadowBias.y / (((zdis)) * (_LightParams.z));

#if defined (SHADER_API_D3D9)
return SAMPLE_DEPTH_TEXTURE_LOD(_ShadowMapTexture, float4(suv.xy, z_buffer_value, 0));			
#else 
...
vr MarchColor(float3 dir, float dis, float3 wpos, float2 uv, float2 InterleavePos, float3 EnterPoint, float3 ExitPoint)
{
	...
	InterleavePos = InterleavePos.xy / 5;
	float index = tex2Dlod(Tile5x5, float4(InterleavePos.xy, 0, 0)).r;
	...
	float rayWorldDis = length(WorldEnter - _WorldSpaceCameraPos) * zScale;
	...
	vr frag(v2f i)
	{
		...
		return MarchColor(rayDis, startT, wpos, uv.xy, fmod((_ScreenParams.xy * VolumeScale) * uv, 5), near, far);
	}
