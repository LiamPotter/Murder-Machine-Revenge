...
public class HxVolumetricCameraEditor : Editor  
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        {          
            ...
            EditorGUILayout.PropertyField(m_WarmUpShaders);
            if (targetCamera.WarmUpShaders == HxVolumetricCamera.ShaderWarm.All)
            {
                EditorGUILayout.HelpBox("NOTE! Expect a once off large loadtime the first time you run the game in the editor! (30s-60s) This load time will never happen again", MessageType.Warning);
            }
            else
            {
                EditorGUILayout.HelpBox("NOTE! Expect a once off large loadtime the first time you build the game! (30s-60s) This load time will never happen again", MessageType.Info);
            }
